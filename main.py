...
root.title('Onuelito Audio Player')
    ...
    def __init__(self, master=None):
        ...
        def listbox_func(evnt):
            ...
            if self.listbox.curselection():
                song = audiobox.song_list[self.listbox.curselection()[0]][1]
                audio = MP3(song)
...
class InputPannel(Frame):
    def __init__(self, master=None):
        ...
        self.volume = Scale(root, length=150, from_=100, to=0); self.volume.set(100)
        ...
        autoplay = Checkbutton(self, text="Autoplay", font=(None, 11), anchor='w', var=self.autoplay, command=lambda: self.loop.set(False))
        loop = Checkbutton(self, text="Loop", font=(None, 11), anchor='w', var=self.loop, command=lambda: self.autoplay.set(False))
...
#Credits
Label(root, text="Made by Onuelito").grid(row=1, column=0)
Label(root, text="Powered by Pygame, Mutagen and Tkinter", anchor="w").grid(row=2, column=0)
Label(root, text="Itch.io : Onuelito", fg="blue").grid(row=3, column=0)
Label(root, text="Twitter : @OGplufox", fg="blue").grid(row=4, column=0)
...
createTab("File", [["Add Track(s)", lambda: audiobox.add_song(filedialog.askopenfilenames(filetypes=[('Music files', '.wav .mp3 .ogg')]))], 
                ["Add Playlist", lambda: audiobox.add_playlist(filedialog.askdirectory())]], menuBar)
